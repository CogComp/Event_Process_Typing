...
class torchpart(object):
    ...
    def profile_test_verb(self, verbs, sequences, true_ids, v2s, verb_thres=50, length_thres=5, limit_ids=True):
        cand_ids = set([])
        id_count = {}
        for id in true_ids:
            cand_ids.add(id)
            if id_count.get(id) == 0:
                id_count = 1
            else:
                id_count += 1
        ...
        def test(s_vec, v_vec, cand_ids):
            while index.value < len(all_cases):
                ...
                if len(all_cases[id]) >= length_thres:
        ...
        print ("v_count<=", verb_thres, ':', mrr_vl, hits1_vl, hits10_vl)
        print ("v_count>", verb_thres, ':', mrr_vu, hits1_vu, hits10_vu)
...
def main():
    ...
    try:
        ...
        pass
    data_file, data_bin, model_bin, rst_file = '/shared/corpora-tmp/wikihow/wikiHowSubsequences.tsv', '../run/seqVerbMC/data_subsrl_1sv_1sa_argtrim.bin', './seqSSmrl_subsrl/RobertaVerbMC/', './seqSSmrl_subsrl/results_subsrl_1sv_1sa_a' + str(this_alpha) + '_m' + str(this_margin) + '_m' + str(this_margin2) + '_wbias_' + time.strftime('%H_%d-%m-%y') + '.txt'
    data = Data()
    ...
    for train_index, test_index in rs.split(indices):
        with open('./recover_test_index_fold'+str(fold)+'.txt', 'w') as fp2:
            for w in test_index:
                fp2.write(str(w) + '\n')
            fold += 1
